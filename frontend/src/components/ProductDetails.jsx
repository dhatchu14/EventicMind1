...
const ProductDetails = () => {
    ...
    useEffect(() => {
        const fetchDetails = async () => {
            ...
            try {
                ...
                setProductError(productError || generalError);
    ...
    const handleAddToCart = () => {
        // Check if product data is loaded and stock is valid
        if (product && typeof inventoryStock === 'number' && inventoryStock > 0) {
            ...
            if (quantity > inventoryStock) {
                 ...
                 setQuantity(inventoryStock); // Reset quantity to max available
                 return;
            }
            ...
            // Show error if product/stock is unavailable or invalid
          ...
          <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
            ...
            <div className="lg:w-1/2">
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 border border-gray-200 dark:border-gray-700">
                ...
                {/* Price */}
                <div className="mb-6"><span className="text-3xl font-bold text-black dark:text-white">${product.price.toFixed(2)}</span></div>

                {/* Stock Status */}
                <div className="mb-6 flex items-center gap-2">
                    <Box className="h-5 w-5 text-muted-foreground" /><span className="font-medium">Availability:</span>{renderStockStatusBadge()}
                </div>
                ...
                <div className="mb-6">
                  ...
                  <div className="flex items-center">
                    <Button variant="outline" size="icon" onClick={() => handleQuantityChange('decrease')} disabled={quantity <= 1} aria-label="Decrease quantity" className="border-gray-300 dark:border-gray-600 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer"><MinusCircle className="h-5 w-5" /></Button>
                    <span className="mx-4 w-10 text-center font-medium text-lg">{quantity}</span>
                    <Button variant="outline" size="icon" onClick={() => handleQuantityChange('increase')} aria-label="Increase quantity" className="border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer" disabled={loadingInventory || (typeof inventoryStock === 'number' && quantity >= inventoryStock)}><PlusCircle className="h-5 w-5" /></Button>
                  </div>
                  {!loadingInventory && typeof inventoryStock === 'number' && inventoryStock > 0 && inventoryStock <= 10 && (<p className="text-xs text-yellow-600 dark:text-yellow-400 mt-2">Only {inventoryStock} left in stock!</p>)}
                </div>

                {/* Add to Cart Button */}
                <div className="mb-6">
                  <Button className="w-full bg-black hover:bg-gray-800 dark:bg-white dark:hover:bg-gray-200 text-white dark:text-black cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed" size="lg" onClick={handleAddToCart} disabled={isAddToCartDisabled}>
                    {addToCartText}
                  </Button>
                </div>
                ...
                {/* Description Preview */}
                <div className="prose prose-sm dark:prose-invert max-w-none"><p>{product.description ? `${product.description.substring(0, 150)}${product.description.length > 150 ? '...' : ''}` : 'No description available.'}</p></div>
              </div>
            </div>
          </div>
